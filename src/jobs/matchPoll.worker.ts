...
import type { MatchPollJobData, MatchInfo } from '../types/index.js';
  ...
  const worker = new Worker<MatchPollJobData>(
    ...
    async (job: Job<MatchPollJobData>) => {
      ...
      try {
        ...
        const recentMatches = await accountService.getRecentMatches(
          platformEnum,
          platformAccountId
        );
        ...
        const newMatches = recentMatches.filter(
          (m: MatchInfo) => m.matchId !== lastMatchId
        );
