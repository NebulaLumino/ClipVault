...
discordClient.on('ready', async () => {
    ...
    try {
        await discordClient.rest.put(Routes.applicationCommands(config.DISCORD_CLIENT_ID), { body: allCommands.map(cmd => cmd.toJSON()) });
...
discordClient.on('interactionCreate', async (interaction) => {
    ...
    try {
        ...
        let user = await userService.getOrCreateUser(discordUser.id, discordUser.username, discordUser.globalName || undefined);
        if (!user) {
            ...
            case 'settings': {
                ...
                switch (subcommand) {
                    ...
                    case 'clip-types': {
                        ...
                        const clipTypes = types?.split(',').map((t) => t.trim().toLowerCase()) || [];
                        await userService.updatePreferences(user.id, {
                            preferredClipTypes: clipTypes,
                        });
                        await interaction.reply({
