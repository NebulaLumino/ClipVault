...
export class SteamClient {
    ...
    async getDota2MatchHistory(steamId, limit = 5) {
        const url = `https://api.opendota.com/api/players/${steamId}/matches?limit=${limit}`;
        try {
            const response = await fetch(url);
            if (!response.ok) {
                throw new SteamError("Failed to fetch Dota 2 match history", "REQUEST_FAILED", response.status);
            }
            const matches = (await response.json());
            return matches.map((m) => ({
                matchid: String(m.match_id),
            ...
            if (error instanceof SteamError)
                throw error;
            logger.error("Dota 2 API error", { error: String(error) });
            throw new SteamError(String(error), "REQUEST_FAILED");
        }
    }
    convertSteam64ToSteam32(steam64) {
